	...
	run: async (client, interaction, args) => {
		...
		await interaction.reply({embeds: [
			new MessageEmbed()
				...
				.setColor("#2f3136")
				.setFooter({text: "Système de tickets by: Akanytb", iconURL: client.user.displayAvatarURL({dynamic: true})})
		], components: [
			...
			if (button === "transcript-channel") {
				...
				if (!guildData) {
					return interaction.editReply({embeds: [
						new MessageEmbed()
							.setTitle("Système de tickets \🔴")
							.setDescription("This server doesn't have configurated the Système de tickets!")
							.setColor("RED")
							.setFooter({text: "Système de tickets by: Akanytb", iconURL: client.user.displayAvatarURL({dynamic: true})})
					], components: []});
				}
